...
contract('UpgradeabilityProxy', function(accounts){
    ...
    it('18. Initiate Proxy and change Token version',  function(){
        Registry.new({from: accounts[0]}).then(function (value1) {
            ...
            scn_success = value.valueOf();
            return upgrad_proxy.balanceOf(accounts[1],{from: accounts[0]});
        }).then( function (value) {
            ...
            assert.equal(scn_success, true, "Second Transaction Not Successful");
            assert.equal(value.valueOf(), 202, "Unexpected value");
        });
    });
});